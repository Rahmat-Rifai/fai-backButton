...
/// <summary>
/// Low-level mouse hook (WH_MOUSE_LL) untuk menangkap klik kanan.
...
internal sealed class MouseHook : IDisposable
{
    ...
    /// <summary>
    /// Event dipanggil saat mouse event terjadi.
    /// Parameter 1: message id (mis. WM_RBUTTONDOWN).
    /// Return true untuk block event, false untuk pass-through.
    /// </summary>
    public event Func<int, NativeMethods.MSLLHOOKSTRUCT, bool>? OnMouseAction;
    ...
    private IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam)
    {
        if (nCode >= 0)
        {
            ...
            bool blocked = OnMouseAction?.Invoke(wParam.ToInt32(), hookStruct) ?? false;
            if (blocked)
                return new IntPtr(1);
        }
